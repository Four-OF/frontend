...
function LearningPathStep({ module, index, arrayIndex, displayIndex, currentIndex }) {
 ...
 function LearningPathProgress({
  ...
  // UPDATED: Initialize quiz progress on component mount and when languageData changes
  useEffect(() => {
    if (languageData && languageData.length > 0 && !initialized) {
      loadProgress();
      setInitialized(true);
    }
  }, [languageData, initialized, userLanguage]);
  ...
  }, [userLanguage]); // Added userLanguage to dependency

  // Also check for URL parameter that indicates completion
  useEffect(() => {
    const completedModule = searchParams.get('complete');
    if (completedModule) {
      // Reload progress when coming back with completion parameter
      loadProgress();
    }
  }, [searchParams, userLanguage]); // Added userLanguage to dependency
  ...
  const handleResetProgress = () => {
    // Ensure userLanguage is defined and not null/undefined
    if (!userLanguage) {
      console.error("userLanguage is undefined or null");
      return;
    }

    const modulesToReset = languageData?.chapters ?
      languageData.chapters.map((chapter, index) => ({
        id: chapter.chapter_number.toString(),
        title: chapter.topics[0]?.name || `Chapter ${chapter.chapter_number}`,
        description: chapter.topics[0]?.name ? `Learn ${chapter.topics[0].name.toLowerCase()}` : `Learn key concepts in chapter ${chapter.chapter_number}`,
        status: index === 0 ? "in-progress" : "locked",
        lessons: chapter.topics.reduce((acc, topic) => acc + (topic.learning_data?.length || 0), 0),
        completedLessons: 0,
      })) :
    ...
    localStorage.setItem(`quizProgress_${userLanguage}`, JSON.stringify(resetProgress));
  };
  ...
  if (!initialized || !languageData || languageData.length === 0) { // Show loading until languageData is fetched and initialized
    return <div className="flex justify-center items-center h-64">Loading language content...</div>
  }
  // Calculate overall progress
  // const totalLessons = modules.reduce((acc, module) => acc + module.lessons, 0)
  // const completedLessons = modules.reduce(
      ...
      <div className="relative pb-16 pt-8 min-h-screen">
        ...
        <div className="relative z-10 space-y-12">
          {modules.map((module, index) => (
            <LearningPathStep
              key={module.chapter_number || module.id} // Use chapter_number or id as key
              module={module}
              index={index + 1} // Pass 1-based index to content
              arrayIndex={index}
